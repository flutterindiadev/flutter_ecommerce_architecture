...
class OrderItemResponseEntity
  ...
  OrderItemResponseEntity restore(OrderItem data) {
    final productEntity = ProductEntity(id: data.product.id, currencyId: data.product.currencyId,
        name: data.product.name, description: data.product.description, imageUrl: data.product.imageUrl,
        category: data.product.category, price: data.product.price);
    final addressEntity=AddressEntity(id: data.address.id, streetName: data.address.streetName,
        country: data.address.country, city: data.address.city, pincode: data.address.pincode,
        floor: data.address.floor, building: data.address.building, apartment: data.address.apartment);
    return OrderItemResponseEntity(OrderItemEntity(orderId: data.orderId,
        paymentMode: data.paymentMode,
        deliveryDate: data.deliveryDate,
        address: addressEntity, product: productEntity));
  }

  @override
  OrderItem transform() {
    return OrderItem(product: orderItemEntity.product.transform(), orderId: orderItemEntity.orderId,
        paymentMode: orderItemEntity.paymentMode,
        deliveryDate: orderItemEntity.deliveryDate, address: orderItemEntity.address.transform());
  }
}